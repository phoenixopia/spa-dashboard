...
export default function Testimonial() {
  ...
  const [newTestimonial, setNewTestimonial] = useState({ name: "", description: "", image: "" });
  ...
  const handleAddTestimonialChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const handleAddImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      const imageUrl = URL.createObjectURL(files[0]);
      setNewTestimonial(prev => ({ ...prev, image: imageUrl }));
    }
  };

  const handleAddSave = () => {
    setData(prev => [...prev, newTestimonial]);
    setAddModal(false);
  };
            ...
            <div className="relative overflow-x-auto shadow-sm rounded-lg">
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    <th className="px-6 py-3">Testimonial Name</th>
  ...
  <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex items-center justify-center z-50 px-4 sm:px-6">
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 w-full max-w-md dark:bg-gray-900">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Testimonial Name</label>
          <input
            name="name"
            type="text"
...
{showModal && (
  <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex items-center justify-center z-50 px-4 sm:px-6">
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 w-full max-w-md dark:bg-gray-900">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-white">Edit Testimonial</h2>
        <button
