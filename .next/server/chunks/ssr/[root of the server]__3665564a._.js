...
function UserSettings({ userId, initialName, initialEmail }) {
    ...
    const [tempValue, setTempValue] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])(initialName || ''); // Ensure tempValue is never undefined or null
    ...
    const closeModal = ()=>{
        setModalOpen(false);
        setEditField(null);
        setTempValue(initialName || ''); // Reset tempValue on modal close
        setOldPassword('');
        setNewPassword('');
        setConfirmPassword('');
    };
    const handleChange = async ()=>{
        ...
        try {
            ...
            if (editField === 'name' && tempValue.trim()) {
                updatePayload.name = tempValue;
            } else if (editField === 'email' && tempValue.trim()) {
                updatePayload.email = tempValue;
            } else if (editField === 'password') {
                if (!validatePassword(newPassword)) {
                    alert('Password must be at least 8 characters long, include an uppercase letter and a special character.');
                    setLoading(false);
                    return;
                }
            ...
            const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].put(`${BURL}/user/${userId}`, updatePayload);
            // Update local state on success
            if (editField === 'name') setName(tempValue);
            if (editField === 'email') setEmail(tempValue);
            if (editField === 'password') alert('Password updated!');
            closeModal();
        } catch (error) {
